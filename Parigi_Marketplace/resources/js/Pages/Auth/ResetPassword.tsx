...
export default function ResetPassword({ token, email }: { token: string; email: string }) {
    ...
    const inputCls = (field: keyof typeof errors) =>
        `w-full rounded-lg border px-4 py-2.5 text-sm text-white placeholder-white/30 outline-none transition focus:ring-2 focus:ring-[#74c69d]/40
        ${errors[field] ? 'border-red-400/60 bg-red-900/20' : 'border-white/10 bg-white/10 focus:border-[#74c69d]/60'}`;
                ...
                <div className="relative z-10 w-full max-w-md px-4 py-8">
                    ...
                    >
                        ...
                        <div className="mb-7 text-center">
                            ...
                            <p className="mt-1 text-sm text-white/50">Buat password baru</p>
                        </div>
                        ...
                        <form onSubmit={submit} className="space-y-4">
                            ...
                            {/* Email (readonly) */}
                            <div>
                                <label htmlFor="email" className="mb-1.5 block text-sm font-medium text-[#d8f3dc]">Email</label>
                                <input
                                    id="email"
                                    type="email"
                                    value={data.email}
                            ...
                            {/* Password */}
                            <div>
                                <label htmlFor="password" className="mb-1.5 block text-sm font-medium text-[#d8f3dc]">Password Baru</label>
                                <input
                                    id="password"
                                    type="password"
                                    value={data.password}
                            ...
                            {/* Confirm Password */}
                            <div>
                                <label htmlFor="password_confirmation" className="mb-1.5 block text-sm font-medium text-[#d8f3dc]">
                                    Konfirmasi Password
                                </label>
                                <input
                                ...
                                />
                                {errors.password_confirmation && <p className="mt-1 text-xs text-red-300">{errors.password_confirmation}</p>}
                            </div>
